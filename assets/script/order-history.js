document.addEventListener("DOMContentLoaded", function () {
    ...
    if (!username) {
        orderList.innerHTML = "<li>Vui lòng <a href='login.html'>đăng nhập</a> để xem lịch sử đặt hàng.</li>";
        return; // Dừng lại nếu chưa đăng nhập
    }

    // Hiển thị tên tài khoản
    userInfoElement.textContent = `Xin chào, ${username}!`; // Hiển thị tên tài khoản
    ...
    if (orders.length === 0) {
        orderList.innerHTML = "<li>Không có đơn hàng nào.</li>"; // Hiển thị thông báo nếu không có đơn hàng
    } else {
        ...
        orders.forEach(order => {
            ...
            orderInfo.textContent = `Đơn hàng: ${order.name}, Giá: ${formatCurrency(order.price)}, Số lượng: ${order.quantity}`;
